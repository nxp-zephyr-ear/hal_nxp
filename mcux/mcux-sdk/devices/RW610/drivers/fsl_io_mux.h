...
/*! @name Driver version */
/*@{*/
/*! @brief IO_MUX driver version 2.1.0. */
#define FSL_IO_MUX_DRIVER_VERSION (MAKE_VERSION(2, 1, 0))
/*@}*/
...
typedef enum
{
    IO_MUX_PinConfigNoPull   = 0U,
    IO_MUX_PinConfigPullUp   = 1U,
    IO_MUX_PinConfigPullDown = 2U,
} io_mux_pin_config_t;
...
static inline void IO_MUX_SetPinConfig(uint32_t pin, io_mux_pin_config_t config)
{
    ...
    if (pin < 22U || pin > 27U)
    {
        reg = &SOCCTRL->PAD_PU_PD_EN0 + pin / 16U;
    }
    else
    {
        reg = &AON_SOC_CIU->PAD_PU_PD_EN1;
    }

    *reg = (*reg & ~(3UL << shift)) | ((uint32_t)config << shift);
}
...
static inline void IO_MUX_SetRfPinOutLevelInSleep(uint32_t pin, io_mux_sleep_pin_level_t level)
{
    ...
    else
    {
        SOCCTRL->PAD_RF_SW_SLP_CONFIG =
            (SOCCTRL->PAD_RF_SW_SLP_CONFIG & ~mask) | (1UL << pin) | (((uint32_t)level) << (pin + 16));
    }
}
