 ...
 *  @param  cfg       : t3pll_mci_48_60m_irc clock configuration
 */
void CLOCK_InitT3RefClk(clock_t3_mci_irc_config_t cfg)
{
...
static void CLOCK_PowerUpAnaGrp(void)
{
    ...
    if ((anaGrpPu & 0x1000U) == 0U)
    {
        SYSCTL2->ANA_GRP_CTRL = anaGrpPu | 0x1000U;
...
/*! @brief  Initialize the AVPLL.
 *  @param  cfg       : AVPLL clock configuration
 */
void CLOCK_InitAvPll(const clock_avpll_config_t *cfg)
{
    assert(cfg);
    ...
    CLOCK_ConfigAvPll();
    CLOCK_ConfigAvPllCh1(cfg->ch1Freq);
    CLOCK_ConfigAvPllCh2(cfg->ch2Freq);
    ...
    /* Enable channels */
    CLOCK_EnableAvPllCh(true, true, cfg->enableCali);
}
